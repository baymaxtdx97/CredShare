...
class MalayReview extends StatelessWidget {
  const MalayReview({
    Key key,
    @required this.user
  }): super(key: key);
    ...
    return Scaffold(
      body: new Stack(
        ...
        children: <Widget>[
          ...
          StreamBuilder(
              stream:Firestore.instance.collection("bfast_reviews").document("malay").snapshots(),
                      ...
                      return Column(
                        children: <Widget>[
                          Padding(
                            ...
                            child: Text(snapshot.data["Malay_rating"][position].toString().substring(0,1) + "/5",
                              style: TextStyle(
                          ...
                          Padding(
                            padding:
                            const EdgeInsets.all(2),
                            child: Text(snapshot.data["Malay_review"][position].toString().substring(0, snapshot.data["Malay_review"][position].toString().length - DateTime.now().toString().length),
                              style: TextStyle(
                                  fontSize: 16.0, fontWeight: FontWeight.bold,color: Color(0xff2d386b)),
                            ),
                          ),
