...
class VegetarianPage2 extends StatefulWidget {
  const VegetarianPage2({
    Key key,
    @required this.user
  }): super(key: key);
  final FirebaseUser user;
  @override
  _VegetarianPage2State createState() => new _VegetarianPage2State();
}

class _VegetarianPage2State extends State<VegetarianPage2> {
    ...
    if(formState.validate()) {
      ...
      try {
        Firestore.instance.collection("dinner_reviews")
            .document("vegetarian")
            .updateData({"Vegetarian_rating": FieldValue.arrayUnion(
            [_stars+DateTime.now().toString()])});
        Firestore.instance.collection("dinner_reviews")
            .document("vegetarian")
            .updateData({"Vegetarian_review": FieldValue.arrayUnion(
