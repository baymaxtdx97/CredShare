...
class MalayPage2 extends StatefulWidget {
  const MalayPage2({
    Key key,
    @required this.user
  }): super(key: key);
  ...
  @override
  _MalayPage2State createState() => new _MalayPage2State();
}

class _MalayPage2State extends State<MalayPage2> {
    ...
    if(formState.validate()) {
      ...
      try {
        Firestore.instance.collection("dinner_reviews")
            .document("malay")
            .updateData({"Malay_rating": FieldValue.arrayUnion(
            [_stars+ DateTime.now().toString()])});
        Firestore.instance.collection("dinner_reviews")
            .document("malay")
            .updateData({"Malay_review": FieldValue.arrayUnion(
