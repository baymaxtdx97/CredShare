...
class NoodlePage2 extends StatefulWidget {
  const NoodlePage2({
    Key key,
    @required this.user
  }): super(key: key);
  ...
  @override
  _NoodlePage2State createState() => new _NoodlePage2State();
}

class _NoodlePage2State extends State<NoodlePage2> {
    ...
    if(formState.validate()) {
      ...
      try {
        Firestore.instance.collection("dinner_reviews")
            .document("noodle")
            .updateData({"Noodle_rating": FieldValue.arrayUnion(
            [_stars+ DateTime.now().toString()])});
        Firestore.instance.collection("dinner_reviews")
            .document("noodle")
            .updateData({"Noodle_review": FieldValue.arrayUnion(
