...
class SharingPage2 extends StatefulWidget {
  const SharingPage2({
    Key key ,
    @required this.user
  }) : super(key: key);
  ...
  @override
  _SharingPage2State createState() => new _SharingPage2State();
}


class _SharingPage2State extends State<SharingPage2> {
                                ...
                                child: TextFormField(
                                  ...
                                  validator: (input) {
                                    if (input.isEmpty) {
                                      ...
                                      // ignore: missing_return,
                                    } else if (int.parse(input)+ snapshot.data['dinner_credits_used']>3) {
                                      return 'You have exceeded your allowed amount by '
                                          + (int.parse(input)+snapshot.data['dinner_credits_used']-3).toString();
                                    }
                                  } ,
                                    ...
                                    try{
                                      ...
                                      while (counter>0){
                                        Firestore.instance.collection("users").document(_receiver2).updateData(
                                            {"dinner_credits_shared": FieldValue.arrayUnion([snapshot.data["dinner_credits_expiry"][counting]])});
                                        Firestore.instance.collection("users").document(widget.user.uid).updateData(
                                            {"dinner_credits_expiry":FieldValue.arrayRemove([snapshot.data["dinner_credits_expiry"][counting]])});
                                        counter-=1;
                                        counting+=1;
                                      }
                                      Firestore.instance.collection("users").document(widget.user.uid).updateData(
                                          {"dinner_credits_used": FieldValue.increment(int.parse(_number))});
                                      // Below is the updating of recent
                                      Firestore.instance.collection("users").document(widget.user.uid).updateData(
                                          {"recent_transactions_credits": FieldValue.arrayUnion(
                                              ["-"+_number+" Dinner"+ " "+DateTime.now().toString()])});
                                      Firestore.instance.collection("users").document(widget.user.uid).updateData(
                                      ...
                                      Firestore.instance.collection("users").document(widget.user.uid).updateData(
                                          {"recent_transactions_name": FieldValue.arrayUnion([
                                            _receiver + " " + DateTime.now().toString()])});
                                      // Below is the receiver side
                                      Firestore.instance.collection("users").document(_receiver2).updateData(
                                          {"recent_transactions_credits": FieldValue.arrayUnion(
                                              ["+"+_number+" Dinner"+ " "+ DateTime.now().toString().substring(0,16)])});
                                      Firestore.instance.collection("users").document(_receiver2).updateData(
